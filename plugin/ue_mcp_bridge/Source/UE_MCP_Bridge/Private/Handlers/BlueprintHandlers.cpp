...
void FBlueprintHandlers::RegisterHandlers(FMCPHandlerRegistry& Registry)
{
	...
	// authoring (read-side coverage was enabled above; mutations live on the
	// same source file so the whole Blueprint editing surface is available).
	...
	Registry.RegisterHandlerWithTimeout(TEXT("compile_blueprints"), &CompileBlueprints, 240.0f);
